...
public class ControllerCommands : MonoBehaviour
{
    ...
    private void Start()
    {
        battleManager = GameObject.Find("EnemySpawner");
    ...
    void Update()
    {
        ...
        if (difficultyModifier > 200 && battleManager.GetComponent<BattleManager>().lightningAttackCount < 1)
        {
        ...
        if (difficultyModifier > 400 && battleManager.GetComponent<BattleManager>().lightningStrikeCount < 1)
        {
        ...
        if (difficultyModifier > 540 && battleManager.GetComponent<BattleManager>().lightningBoltCount < 1)
        {
        ...
        if (difficultyModifier > 660 && battleManager.GetComponent<BattleManager>().powerHitCount < 1)
        {
        ...
        //                      
        if (difficultyModifier > 740 && battleManager.GetComponent<BattleManager>().ultimateCount < 1)
        {
        ...
        if (stickVector.y <= -0.93 && canLightningStrike && strikeCount < 1 && directInteractor.selectTarget == null)
        {
            strikeCount++;
        ...
        if (directInteractor.selectTarget)
       {
        ...
        if (directInteractor.selectTarget)
        {
            if (directInteractor.selectTarget.name == "Mjolnir") // if that thing is Mjolnir...
            {
                ...
                // IF Player pushes JOYSTICK DOWN, Lightning Strike
                if (stickVector.y <= -0.93 && canLightningStrike && strikeCount < 1 && hammerHand)
                {
                    strikeCount++;
    ...
    {
        battleManager.GetComponent<BattleManager>().lightningAttackCount++; //Increases lightningAttack count
    ...
    IEnumerator CallLightningStrikeCooldown()
    {
        ...
        battleManager.GetComponent<BattleManager>().lightningStrikeCount++; //Increases LightningStrike count
    ...
    {
        battleManager.GetComponent<BattleManager>().lightningBoltCount++; //Increases lightningBolt count
    ...
    {
        battleManager.GetComponent<BattleManager>().powerHitCount++; //Increases PowerHit count
        emitterPS.Play();
        yield return new WaitForSeconds(10);
    ...
    {
        battleManager.GetComponent<BattleManager>().ultimateCount++; //Increases Ultimate count
        ...
        yield return new WaitForSeconds(20); // wait 20 seconds so player can attack
        ...
        yield return new WaitForSeconds(90); // how long to wait before can do Ultimate again
